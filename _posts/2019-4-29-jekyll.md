...
1. Linquid sytax  
    1.1. Objects  
    Objects tell Liquid where to output content. They’re denoted by double curly braces（中括号替换为花括号）: `[[` and `]]` . For example:  
   `[[ page.title ]]`  
    1.2. Tags  
    Tags create the logic and control flow for templates. They are denoted by curly braces and percent signs（中括号替换为花括号）: `[%` and `%]`
    For example:
    ...
    [% include XXX %] 调用 _include 文件夹中XXX文件   #中括号替换为花括号
    ...
    [% highlight ruby linenos%][% endhighlight %] 代码高亮以及显示行数 #（中括号替换为花括号）
    ...
    |   strip_html            |     将HTML标签去除                   |       
    |   truncate:300          |      截取300字符                     |   
    |   date: "%B %-d, %Y"    |       时间格式                       |   
