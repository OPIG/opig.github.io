...
1. Linquid sytax  
    1.1. Objects  
    Objects tell Liquid where to output content. They’re denoted by double curly braces: `\{\{` and `\}\}` (ignore the '\\', for compile issue). For example:  
   `{{ page.title }}`  
    1.2. Tags  
    Tags create the logic and control flow for templates. They are denoted by curly braces and percent signs: `\{\%` and `\%\}` (ignore the '\\', for compile issue)
    For example:
...
You can set 'title', 'layout', 'tags', etc. variables for the page here.  
