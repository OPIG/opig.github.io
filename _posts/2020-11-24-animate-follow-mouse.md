...
When we browse blogs online, we always find some blogs have add some animation when we click or move mouse. Here is one sample source code, you can copy this or try it by yourself. 😄
      ...
      $("body").on('click', function (e) {
        ...
        $(this).append(
          `<div class="test test${id}" 
            ...
            ${Toast[Math.floor(Math.random() * Color.length)]}
          </div>`)

        $(`.test${id}`).css({ 'left': location.x - 20, 'top': location.y })
        .animate({ top: location.y - 100, opacity: 0, fontSize: 10 }, 1000);
